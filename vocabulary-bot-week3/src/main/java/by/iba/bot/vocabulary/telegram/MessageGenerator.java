...
public class MessageGenerator {
    ...
    @Getter
    private String successCancelMessage;
    @Getter
    private String errorNotifySetMessage;
    @Getter
    public String errorWordSetMessage;
    ...
    private String generateHelpMessage() {
        var message = botConfig.getCommands().stream().reduce(":bulb: <b>Доступные команды</b>\n", (m, c) -> m + c.getName() + " - " + c.getDescription() + "\n", (m1, m2) -> m1);
        return EmojiParser.parseToUnicode(message);
    }

    private String generateSuccessCancelMessage() {
    ...
    private String generateErrorNotifySetMessage() {
    ...
    private String generateErrorWordSetMessage() {
    ...
    public String generateStatusMessage(SessionStatus ss) {
        ...
        message += "Автоматический показ новых слов: <b>" + (ss.isShowWords() ? "ВКЛ" : "ВЫКЛ") + B;
        message += "Периодичность показа слов: <b>" + (ss.getShowInterval() != null ? ss.getShowInterval() + " минут(ы)" : "не задан") + B;
        message += "Количество слов показываемых за один раз: <b>" + ss.getNumOfWordsToShow() + B;
        message += "Предыдущее уведомление отправлено: <b>" + (currentPeriod != null ? currentPeriod + " минут(ы) назад" : "никогда") + B;
        return EmojiParser.parseToUnicode(message);
    }

    public String generateWordsMessage(List<Word> words) {
        return EmojiParser.parseToUnicode(":open_book:\n" + words.stream().map(this::generateWordMessage).collect(Collectors.joining()));
    }

    public String generateWordMessage(Word word) {
        StringBuilder sb = getWordDescription(word);
    ...
    private void addWordTranslation(Word word, StringBuilder sb) {
    ...
    private StringBuilder getWordDescription(Word word) {
    ...
    public String generateAboutMessage() throws UnknownHostException {
        StringBuilder sb = new StringBuilder(":information_source: <b>Информация</b>\n");
        sb.append(":black_small_square: ").append(botConfig.getVocLang()).append(" словарь");
        sb.append("\n");
        ...
        sb.append("\n");
        sb.append(":black_small_square: Сервер: <b>").append(InetAddress.getLocalHost().getHostName()).append("</b>");
        sb.append("\n");
        sb.append("\n");
        sb.append(":man_technologist: Автор: <b>").append("@IBA10_VocabularyBot").append("</b> © ").append(Year.now().getValue());
